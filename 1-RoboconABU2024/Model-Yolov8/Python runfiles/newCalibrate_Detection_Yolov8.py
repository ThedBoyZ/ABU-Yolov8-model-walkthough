...
def save_image_with_predictions(frame, centers, dists):
    ...
    cv2.imwrite(f"{output_dir}/image_{timestamp}.jpg", frame)

    # Write the predictions to a text file
    with open(f"{output_dir}/predictions_{timestamp}.txt", "w") as file:
        for center, dist in zip(centers, dists):
            file.write(f"Center: {center}, Distance: {dist} m\n")
